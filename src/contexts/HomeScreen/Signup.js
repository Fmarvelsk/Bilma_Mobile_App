...
export default function SignUpProfile({ setSignUp }) {
  ...
  function createBussiness() {
    ...
    if (!photo)
      return setErrorMsg(
        "Pls upload a business logo, advert flyer or a picture of yourself"
      );
    setLoading(true);
  ...
  async function sendToStorage(uri) {
    ...
    try {
      ...
      const uploadTask = storage
        .ref()
        .child("images/" + `userProfileId ${imgName}`)
        .put(blob, metadata);
  ...
  return (
    <>
      {selectedValue === "user" ? (
        ...
        <>
          <Text style={styles.headerText}>Register Business</Text>
          {showBusiness ? (
            <>
              {isLoading ? (
                <View style={styles.profileImage}>
        ...
        <>
          ...
          <TouchableOpacity
            style={styles.btnView}
            onPress={() => setSelectedValue("merchant")}
          >
